...
export const MediaListingItem: React.FC<any> = ({ _id, props, slug }) => {
  let formattedDate = ' '
  ...
  return (
    <>
      <Link
        href={`/media/${props?.slug?.current}`}
        className="w-full"
        onClick={() => handleButton('select_listing', 'Media Listing', props?.title)}
      >
          ...
          <div className="w-full relative aspect-[16/9]">
            {props?.primaryImg && (
              <Image
                ...
                width={392}
                height={221}
                alt={props?.primaryImg?.alt ?? 'image'}
