...
export const ZoneFilterDropdown: React.FC<any> = ({
  options,
  ...
  const items = options.reduce((initial: any, current: any) => [...initial, current], [])
      ...
      <div className="rounded-[8px] py-3 pr-5 flex gap-2 items-center mb-6 md:mb-0 relative cursor-pointer">
        ...
        {isOpen && (
          <>
            <div
              {...getMenuProps()}
              className="absolute top-[55px] left-0 flex flex-col w-[180px] z-50 rounded-[8px] bg-off-white shadow-level2 overflow-hidden"
            >
