...
export const PopupFilter: React.FC<any> = ({ setFilters, filters, options, centers }) => {
  const [selectedItems, setSelectedItems] = useState<string[]>([])
  const [selectedalphaItems, setSelectedalphaItems] = useState<string>('A-Z')
  const [showFilterPopup, setShowFilterPopup] = useState(false)
  const [filterActive, setFilterActive] = useState(false)
  useEffect(() => {
    if (filters?.sortBy?.zone) {
      setSelectedItems(filters?.sortBy?.zone)
    }
  }, [filters])
  const handleApply = (zone: any, alpha: any) => {
    ...
    GTAEvent('apply_filter', {
      listing_name: 'Clinics',
      filter_options_1: zone !== '' ? zone : null,
      filter_options_2: alpha !== '' ? alpha : null,
    })
  ...
  const handlecheckbox = (eve: any, e: any) => {
          ...
          <div className="bg-off-white w-[80%] md:bg-[#000000b3] fixed inset-y-0 right-0 z-[1001]">
            ...
            >
              <div className="w-full h-full bg-off-white md:max-w-[392px] md:rounded-[12px] mx-auto">
                ...
                <div className="px-2 flex flex-col gap-8 overflow-scroll overflow-x-hidden dropdown-scroll h-[93vh] md:max-h-[333px] md:min-h-[333px]">
                  {/* Alphabetical filter */}
                  <div className="left-0 flex flex-col w-[180px] overflow-hidden">
                    <ul className="relative max-h-[355px] overflow-auto filter-scrollbar">
                  ...
                  <div className="border-[1px] border-grey-3 " />
                  {/* Zone */}
                  <div className="pb-8 left-0 flex flex-col w-[180px] overflow-hidden">
                    ...
                    <ul className="relative max-h-[355px] overflow-auto filter-scrollbar">
                      {options.map((item: any, index: any) => (
                        <label
                          key={index}
                          className={clsx('py-1 pl-4 pr-1 flex items-center cursor-pointer')}
                          onClick={(eve) => handlecheckbox(eve, item)}
                        >
                          <input
                            type="checkbox"
                ...
                <div className="w-full flex flex-row justify-end items-center fixed md:static bottom-0 border-t-[1px] border-grey-3 py-4 px-6 ">
                  <div className="w-1/2">
                    <p
                  ...
                  <button
                    ...
                    onClick={() => handleApply(selectedItems, selectedalphaItems)}
                  >
                    Apply
                  </button>
                </div>
