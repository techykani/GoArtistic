...
import { AllDocProps } from '../search-main'
...
export const AllResult = ({
  ...
  return (
    <div className="max-w-[818px] 2xl:max-w-4xl container mb-10 lg:mb-20">
      <span className="text-base text-grey-8">{paginatedData.length} Results found</span>

      <motion.div className="space-y-[1px]" variants={staggerContainer} initial="from" animate="to">
        {paginatedData.map((props) => (
          <ResultCard key={props._id} {...props} />
        ))}
      </motion.div>
      {/* 
      {allData.length > 1 ? (
