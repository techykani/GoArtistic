...
export const GuidingPrinciple: React.FC<any> = ({ title, description, tagline, points, cta }) => {
  ...
  return (
    <section className="flex flex-col md:gap-12 gap-8 py-12 md:py-[80px] bg-[#FEFEFE]">
      <div className="md:px-[71px] px-6">
        <div ref={headingRef} className="container mx-auto md:flex md:gap-[129px] md:mb-[48px]">
            ...
            {points && (
              <div className=" grid grid-col-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {points?.slice(0, 3).map((points: any, index: any) => (
                  <Link
                    href={points?.link?.href}
                    className={`w-full min-h-[319px] relative h-[319px]`}
                    key={index}
                  >
                    {points?.icon && (
                      <SanityImg
            ...
            {points?.slice(0, 3)?.map((data: any) => (
              <SwiperSlide key={data?.subtitle} className={`!w-[288px] h-full `}>
                <Link href={data?.link?.href}>
                  <div className={`w-full min-h-[319px] relative h-[319px]`}>
