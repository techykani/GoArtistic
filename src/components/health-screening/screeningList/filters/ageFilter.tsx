...
export const AgeFilter: React.FC<any> = ({ filters, setFilters }) => {
  const [selectedItems, setSelectedItems] = useState<string[]>([])

  useEffect(() => {
    if (filters?.selectedGendersAndLanguages?.ageV) {
      setSelectedItems(filters?.selectedGendersAndLanguages?.ageV)
    }
  }, [filters])

  const items = age.reduce((initial: any, current: any) => [...initial, current], [])
  ...
  return (
    <Fragment>
      <div className="rounded-[8px] py-3 pr-3 flex gap-2 items-center mb-6 md:mb-0 relative cursor-pointer">
        <span
        ...
        {isOpen && (
          <>
            <div
              {...getMenuProps()}
              className="absolute top-[44px] left-0 flex flex-col w-[180px] z-50 rounded-[8px] bg-off-white shadow-level2 overflow-hidden"
            >
