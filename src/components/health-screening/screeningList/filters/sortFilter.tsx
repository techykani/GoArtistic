...
export const SortFilter: React.FC<any> = ({ setFilters, filters, consultationFilterVariables }) => {
  const [showDropDrown, setShowDropDown] = useState(false)
  const dropdownRef = useRef<HTMLDivElement>(null)
  const [selectedItems, setSelectedItems] = useState<string>('Price low to high')
  ...
  const handlecheckbox = (eve: any) => {
    ...
    setFilters((prevObject: any) => ({
      ...prevObject,
      selectedSortBy: eve, // update the desired property
    })),
    ...
    <>
      ...
      >
        <span className="flex-1 text-grey-dark text-base leading-[24px] whitespace-nowrap">
          {selectedItems === 'Price low to high' ? 'Low - High' : 'High - Low'}
        </span>
        {showDropDrown ? (
          <svg
        ...
        {/* dropdown */}
        {showDropDrown && (
          <div className="absolute top-[55px] left-0 flex flex-col w-[200px] z-50 rounded-[8px] bg-off-white shadow-level2 overflow-hidden">
