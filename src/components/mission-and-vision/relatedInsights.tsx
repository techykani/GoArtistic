...
export const RelatedInsights = (data: any) => {
    ...
    <div className="bg-[#F1F6FF] py-[36px] md:py-[60px] flex flex-col gap-6">
      <div className="px-6 md:px-[71px]">
        <div ref={headingRef} className="container mx-auto flex flex-col gap-6">
          <Title headingType="h2" theme="light" tagline={data?.title} />
          {windowWidth >= 1050 && (
            <div className="grid grid-cols-3 gap-6">
              {data?.mightValues?.slice(0, 3)?.map((links: any) => (
                <Link
                  href={links?.href ?? ''}
                ...
                >
                  {links?.image && (
                    <div className="w-full max-w-[392px]">
                      <Image
                        src={urlForImage(links?.image)?.url()}
                        width={392}
                        height={221}
                        alt=""
      ...
      {windowWidth < 1050 && (
        <div className="">
          ...
          >
            {data?.mightValues?.map(({ _id, image, title, href }: any) => (
