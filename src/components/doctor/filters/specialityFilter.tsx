...
export const SpecialityFilterDropdown: React.FC<any> = ({ setFilters, filters, specialities }) => {
  const [selectedItems, setSelectedItems] = useState<string[]>([])

  useEffect(() => {
    if (filters?.selectedValue?.speciality) {
      setSelectedItems(filters?.selectedValue?.speciality)
    }
  }, [filters])

  const items = specialities.reduce((initial: any, current: any) => [...initial, current], [])
